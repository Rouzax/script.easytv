...
class TestCheckPremiereExclusionMixIn:
    ...
    def test_both_mixin_fast_path(self, patch_window):
    ...
    def test_both_mixin_regular_episode(self, patch_window):
...
class TestCheckPremiereExclusionSkip:
    ...
    def test_skip_series_excludes_s01e01(self, patch_window):
        patch_window({1: 's01e01'})
    ...
    def test_skip_series_keeps_season_premiere(self, patch_window):
    ...
    def test_skip_series_keeps_regular(self, patch_window):
        patch_window({1: 's02e05'})
        config = RandomPlaylistConfig(premieres=PREMIERE_SKIP, season_premieres=PREMIERE_MIX_IN)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is False

    def test_skip_season_excludes_s02e01(self, patch_window):
        patch_window({1: 's02e01'})
    ...
    def test_skip_season_keeps_series_premiere(self, patch_window):
        patch_window({1: 's01e01'})
        config = RandomPlaylistConfig(premieres=PREMIERE_MIX_IN, season_premieres=PREMIERE_SKIP)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is False
    ...
    def test_skip_season_keeps_regular(self, patch_window):
...
class TestCheckPremiereExclusionOnly:
    ...
    def test_series_only_keeps_s01e01(self, patch_window):
        patch_window({1: 's01e01'})
    ...
    def test_series_only_keeps_season_premiere(self, patch_window):
    ...
    def test_series_only_excludes_regular(self, patch_window):
        patch_window({1: 's02e05'})
        config = RandomPlaylistConfig(premieres=PREMIERE_ONLY, season_premieres=PREMIERE_MIX_IN)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is True
    ...
    def test_series_only_season_skip_excludes_season_premiere(self, patch_window):
        patch_window({1: 's02e01'})
        config = RandomPlaylistConfig(premieres=PREMIERE_ONLY, season_premieres=PREMIERE_SKIP)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is True

    def test_series_only_season_skip_keeps_series_premiere(self, patch_window):
        patch_window({1: 's01e01'})
        config = RandomPlaylistConfig(premieres=PREMIERE_ONLY, season_premieres=PREMIERE_SKIP)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is False

    def test_season_only_keeps_season_premiere(self, patch_window):
        patch_window({1: 's03e01'})
        config = RandomPlaylistConfig(premieres=PREMIERE_MIX_IN, season_premieres=PREMIERE_ONLY)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is False

    def test_season_only_keeps_series_premiere(self, patch_window):
    ...
    def test_season_only_excludes_regular(self, patch_window):
        patch_window({1: 's03e05'})
        config = RandomPlaylistConfig(premieres=PREMIERE_MIX_IN, season_premieres=PREMIERE_ONLY)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is True

    def test_season_only_series_skip_excludes_series_premiere(self, patch_window):
        patch_window({1: 's01e01'})
        config = RandomPlaylistConfig(premieres=PREMIERE_SKIP, season_premieres=PREMIERE_ONLY)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is True

    def test_both_only_keeps_all_premieres(self, patch_window):
        patch_window({1: 's01e01', 2: 's03e01'})
        config = RandomPlaylistConfig(premieres=PREMIERE_ONLY, season_premieres=PREMIERE_ONLY)
        candidates = ['t1', 't2']
        assert _check_premiere_exclusion(1, candidates, config) is False
        assert _check_premiere_exclusion(2, candidates, config) is False
        assert candidates == ['t1', 't2']

    def test_both_only_excludes_regular(self, patch_window):
        patch_window({1: 's02e05'})
        config = RandomPlaylistConfig(premieres=PREMIERE_ONLY, season_premieres=PREMIERE_ONLY)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is True
...
class TestCheckPremiereExclusionEdgeCases:
    ...
    def test_no_episode_data_mixin(self, patch_window):
        patch_window({})  # No data for show 1
        config = RandomPlaylistConfig(premieres=PREMIERE_MIX_IN, season_premieres=PREMIERE_MIX_IN)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is False

    def test_no_episode_data_only_mode(self, patch_window):
        patch_window({})
        config = RandomPlaylistConfig(premieres=PREMIERE_ONLY, season_premieres=PREMIERE_MIX_IN)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is True
    ...
    def test_unparseable_episode_mixin(self, patch_window):
        patch_window({1: 'invalid'})
        config = RandomPlaylistConfig(premieres=PREMIERE_SKIP, season_premieres=PREMIERE_MIX_IN)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is False

    def test_unparseable_episode_only_mode(self, patch_window):
        patch_window({1: 'invalid'})
        config = RandomPlaylistConfig(premieres=PREMIERE_ONLY, season_premieres=PREMIERE_MIX_IN)
        candidates = ['t1']
        assert _check_premiere_exclusion(1, candidates, config) is True
